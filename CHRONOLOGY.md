...
## 10.09.2026 — Авто-синхронизация (день без изменений кода)

**Причина:** плановый ночный cron chrono-агента.

**Что сделано:** за 24 ч (с 10:40 UTC 09.09) боевых коммитов нет — GWT-тираж завершён 09.09, день тихий. Обновлены только артефакты cron: `knowledge_graph/graph.json` + `maintenance_report.json` (rebuild 20:00 UTC 10.09, узлов 397 / рёбер 609). Закрыт SHA строки 09.09 в spec_drift_log.

**Проверка:** bridge :3000 `connected` (uptime ~12.9 ч), extractor :8099 `ok`, `git log --since="24 hours ago"` = только chrono-коммиты. Инфраструктура в норме.
...
**Что сделано:**
- `bot/db.py`: `save_equipment(...)` получил `mode='add'|'replace'`. `add` оставлен дефолтом и сохраняет QA-семантику: разные source-сообщения суммируются, повтор того же `source_message_id` перезаписывает количество без задвоения. `replace` всегда перезаписывает `quantity` и держит `source_message_id=NULL`.
- `bot/ejo_backfill.py`: удалён synthetic sha256 `_equipment_source_message_id`; техника из листа `Персонал и техника` сохраняется через `save_equipment(..., source_message_id=None, mode='replace')`.
- `bot/CONTRACTS.md`: зафиксирована семантика `mode='add'`/`mode='replace'` и запрет синтетических значений в FK `source_message_id`.

**Проверка:** `python3 -m py_compile bot/*.py` — PASS. Monkeypatch без записи в PostgreSQL: повторный `backfill_ejo(bot/templates/ЕЖО_шаблон.xlsx, '2026-09-02')` дважды дал одинаковые строки техники `{Самосвал: 2, Экскаватор: 1, Фронтальный погрузчик: 2, Каток/виброплита: 1}`; все вызовы backfill шли с `mode='replace'` и `source_message_id=None`. Monkeypatch `save_equipment`: два replace-вызова `Экскаватор CAT` quantity `5→2` сохранили итог `quantity=2, source_message_id=NULL`, без суммирования. `python3 -m pytest bot/test_contracts.py bot/test_smoke.py -q` — 16 passed, 5 failed на заранее известных проверках: legacy `bridge_wrapper`, отсутствующий `main_waha.py`, дубликаты personnel, нет процесса `main_waha.py`.
...
- **02.09.2026 15:35** — docs: MASTER_SPEC.md (ЭТАП 1) + синхронизация AGENTS.md под канон 15 таблиц и Human Gate (`d80e1ca`)
- **02.09.2026 15:35** — chrono: синхронизация коммитов 14bc078 + d80e1ca (`ed6aae5`)

- **02.09.2026 16:00** — ejo_backfill: обратный детерминированный разбор ЕЖО `.xlsx` → ОЖР (backfill после обрыва конвейера 21.08). Добавлен `bot/ejo_backfill.py`: лист `Ежедневный отчет` читает A/C/D/J/L/M и пишет факт/план через `save_work_log`; лист `Персонал и техника` пишет позиции организаций через `save_personnel`, технику через `save_equipment`; K853 сохраняется в `ojr_daily_summary.completion_pct` через `get_conn`; фото только считаются из `xl/media`, без импорта в БД. Обновлён `bot/CONTRACTS.md`. Проверка: `python3 -m py_compile bot/*.py` PASS; синтетический dry-run с monkeypatch save/get_conn без записи в PostgreSQL вернул `works_fact=2, works_plan=1, personnel=3, equipment=2, readiness=86.0, photos=0`; `pytest bot/test_contracts.py bot/test_smoke.py -q` = 5 known pre-existing failures (`bridge_wrapper`, `main_waha.py`, personnel duplicates), новых падений нет. Файлы: `bot/ejo_backfill.py`, `bot/CONTRACTS.md`, `CHRONOLOGY.md`.
- **02.09.2026 16:32** — ejo_backfill Round 2: исправлены замечания Grok NEEDS_CHANGES. Готовность больше не читается из K853: строка ищется по D=`Готовность объекта...`, реальный шаблон дал R870/K870=0.3 → `readiness=30.0`. Разбор работ идёт с первой строки ВОР-кода до строки готовности без лимита 852; R853-R868 (8.7.9-8.8.1) включены. Пустой building наследует предыдущий непустой A. Техника пишет `status=None` и стабильный `source_message_id` hash(date,name), персонал backfill вызывает `save_personnel(..., close_existing=False)`, `_org_type` использует только `contractor`. В `CONTRACTS.md` явно зафиксировано: импортируются только суточные L/M, накопления O/P/R/S не импортируются; `photos` — счётчик встроенных `xl/media`, включая логотипы. Проверка без записи в БД на `bot/templates/ЕЖО_шаблон.xlsx`: `works_fact=4, works_plan=0, personnel=11, equipment=4, readiness=30.0, photos=7`; R25 наследует `АБК`, R51 наследует `Галерея`. `python3 -m py_compile bot/*.py` PASS; `pytest bot/test_contracts.py bot/test_smoke.py -q` = 5 pre-existing failures (`bridge_wrapper`, `main_waha.py`, personnel duplicates/process), новых падений нет. Файлы: `bot/ejo_backfill.py`, `bot/db.py`, `bot/CONTRACTS.md`, `CHRONOLOGY.md`.
- **02.09.2026 19:10** — ejo-backfill: обратный разбор ЕЖО .xlsx → ОЖР (восстановление пропущенных дней) (`03088fa`)
- **02.09.2026 19:10** — chrono: синхронизация коммита 03088fa (`6b394af`)
- **02.09.2026 23:00** — chrono: 2026-09-02 — ночная авто-синхронизация; брифинг `briefings/2026-09-02.md` (equipment 15-я таблица, MASTER_SPEC ЭТАП 1, ejo-backfill). Вне скоупа остались: ЕЖО-шаблон (изменён + .bak_0831), `photo_archive/`, `scripts/migrate_passes_to_register.py` (untracked, назначение не проверено).
- **02.09.2026 23:01** — chrono: 2026-09-02 — авто-синхронизация (`9317c4a`)
- **02.09.2026 23:01** — chrono: индекс коммита 9317c4a (`b856a89`)
- **03.09.2026 02:18** — vor-reference: добавлен детерминированный справочник ВОР без LLM-парсинга. В `db/ojr_schema.sql` создана таблица `ojr_vor_reference` (`vor_code` unique, work_name/unit/stage/quantity/unit_price/source/created_at, индексы stage/work_name, COMMENT). В `bot/vor_reference.py` добавлен `load_vor_reference(vor_path=None, priced_path=None, dry_run=False)`: читает `report/templates/ВОР.xlsx`, пропускает заголовки по regex `^\d+(\.\d+)+$`, подхватывает col6 из `ВОР_с_расценками.xlsx`, upsert через `get_conn()` + savepoint на строку; `dry_run=True` не открывает БД. В `bot/db.py` добавлены `get_vor_reference()` и `get_vor_by_code(code)` с `RealDictCursor`. Обновлён `bot/CONTRACTS.md`. Проверка: dry-run `{'total': 573, 'inserted': 0, 'updated': 0, 'with_price': 554}`; monkeypatch `get_conn` на исключение подтвердил отсутствие DB connection в dry-run; `python3 -m py_compile bot/*.py` PASS; `pytest bot/test_contracts.py bot/test_smoke.py -q` = 5 known pre-existing failures (`bridge_wrapper`, `main_waha.py`, personnel duplicates/process), новых падений нет. Файлы: `db/ojr_schema.sql`, `bot/vor_reference.py`, `bot/db.py`, `bot/CONTRACTS.md`, `CHRONOLOGY.md`.
- **03.09.2026 03:37** — vor-reference Round 2: исправлены замечания Grok NEEDS_CHANGES по справочнику ВОР. `load_vor_reference()` получил явную политику дублей: перед upsert читает существующий `vor_code`, при отличии нормализованных `work_name/unit` не затирает строку и увеличивает `skipped_conflict`; при совпадении обновляет объём/метаданные и сохраняет старую цену, если новый импорт пришёл без расценки (`unit_price = COALESCE(EXCLUDED.unit_price, ojr_vor_reference.unit_price)`). `_code()` больше не форматирует float через `.15g`; реальные `report/templates/ВОР.xlsx` проверены — float-кодов нет (`str`: 682, `int`: 8 заголовочных, `None`: 163). `get_vor_by_code()` нормализует вход через `_code()` (`2,1` → `2.1`). Обновлён `bot/CONTRACTS.md`. Проверка: `python3 -m py_compile bot/*.py` PASS; dry-run без записи в БД `{'total': 573, 'inserted': 0, 'updated': 0, 'skipped_conflict': 0, 'with_price': 554}`; `pytest bot/test_contracts.py bot/test_smoke.py -q` = 5 known pre-existing failures (`bridge_wrapper`, `main_waha.py`, personnel duplicates/process), новых падений нет. Файлы: `bot/vor_reference.py`, `bot/db.py`, `bot/CONTRACTS.md`, `CHRONOLOGY.md`.
- **03.09.2026 03:45** — vor-reference: справочник ВОР (573 кода) как таблица ojr_vor_reference + импорт из xlsx (`a2a408c`)
- **03.09.2026 03:45** — chrono: синхронизация коммита a2a408c (`26409ff`)
- **03.09.2026 04:41** — docs: актуализация MASTER_SPEC под ЭТАП 2 (`d711b19`)
- **03.09.2026 05:10** — tests ЭТАП 3: орфаненные v5-контракты переписаны под v6 Hermes Bridge (`messaging.py`/`qa.py` без `bridge_wrapper`, smoke через `:3000/health`, удалён контракт на `main_waha.py`). Добавлены тесты ЭТАП 2: `test_ejo_backfill.py` (dry-run через monkeypatch, наследование building, readiness по тексту), `test_vor_reference.py` (`_code`, dry-run реальных ВОР, skipped_conflict), `test_equipment.py` (`_canon_equipment_name`, `save_equipment` add/replace, `get_daily_equipment`). Проверка: `python3 -m py_compile bot/*.py` PASS; `pytest bot/test_contracts.py bot/test_smoke.py bot/test_ejo_backfill.py bot/test_vor_reference.py bot/test_equipment.py -q` = 29 passed, 1 expected data failure (`test_smoke_personnel_no_leak`); финальный набор из задачи без `test_equipment.py` = 25 passed, 1 expected data failure. Файлы: `bot/test_contracts.py`, `bot/test_smoke.py`, `bot/test_ejo_backfill.py`, `bot/test_vor_reference.py`, `bot/test_equipment.py`, `CHRONOLOGY.md`.
- **03.09.2026 05:35** — tests ЭТАП 3 Round 2: smoke-инвариант персонала переписан под v6-категорийную модель: дубликаты открытых строк теперь ищутся по `(organization_name, full_name, position, start_date)`, чтобы легитимные KPI-категории `ИТР`/`Рабочие`/должности не считались утечкой. Из `fill_ejo.py` удалена мёртвая `_get_aibikon_from_ojr()`: grep по `bot/` показал только определение, а реальный fallback АйБиКон работает через `data_sources.get_aibikon_headcount()` → `_aibikon_ojr_fallback()` → `get_staff()` с `DISTINCT ON (org, position)` и `workers_count`. Данные не менялись; контрольный SQL с новым ключом вернул `groups=0`. Проверка: `python3 -m py_compile bot/*.py` PASS; `cd bot && python3 -m pytest test_contracts.py test_smoke.py test_ejo_backfill.py test_vor_reference.py test_equipment.py -q` = 30 passed, 0 failed. Файлы: `bot/test_smoke.py`, `bot/fill_ejo.py`, `CHRONOLOGY.md`.
...
- **04.09.2026 06:30** — watchdog_bridge: `_load_secrets()` дополнен fallback из профильного `~/.hermes/profiles/alikhan/.env`: `TELEGRAM_BOT_TOKEN` используется только если пуст `ALERT_TELEGRAM_TOKEN`, `TELEGRAM_HOME_CHANNEL` только если пуст `ALERT_TELEGRAM_CHAT_ID`; `DISCORD_WEBHOOK_URL` оставлен только из env/secrets. Парсинг env-файлов вынесен в общий helper. Проверка: `python3 -m py_compile bot/watchdog_bridge.py` PASS; импорт `_load_secrets()` без запуска `run_once()` дал token len 46 и `TELEGRAM_CHAT_ID=-5553424442`; mock `ALERT_TELEGRAM_CHAT_ID=111` подтвердил приоритет secrets. Файлы: `bot/watchdog_bridge.py`, `CHRONOLOGY.md`.
- **04.09.2026 10:24** — fix: buzz-send allowlist в guard_tool_call + watchdog_bridge v2 (`8034650`)
- **04.09.2026 10:36** — sandbox-raw-ingest: добавлен пользовательский Hermes-плагин `~/.hermes/plugins/alikhan-sandbox-raw-ingest` для записи входящих сообщений WhatsApp-песочницы `120363179621030401@g.us` в `bot_memory_messages` как сырья через hook `pre_gateway_dispatch`, до auth/диспетчера и без влияния на delivery (`return None`). Реализованы guards `profile=alikhan` и sandbox chat, production chat не трогается; дедуп по message_id/fallback sha1 через persistent `seen.json` под `flock`; PostgreSQL INSERT с тегами `source=gateway-sandbox-ingest`, `canon=MASTER_SPEC:74 variant-B`; fail-silent логирование. Плагин включён в allowlist профиля `alikhan`. Проверка: `python3 -m py_compile ~/.hermes/plugins/alikhan-sandbox-raw-ingest/__init__.py` PASS; real `PluginManager.discover_and_load(force=True)` зарегистрировал 1 callback модуля `hermes_plugins.alikhan_sandbox_raw_ingest`; smoke через fake MessageEvent сделал INSERT id=4996, повтор с тем же message_id не увеличил count, production/default-profile guards дали 0 вставок, тестовая строка удалена. Файлы: `~/.hermes/plugins/alikhan-sandbox-raw-ingest/plugin.yaml`, `~/.hermes/plugins/alikhan-sandbox-raw-ingest/__init__.py`, `~/.hermes/profiles/alikhan/config.yaml`, `CHRONOLOGY.md`.
- **04.09.2026 10:42** — sandbox-raw-ingest Round 4: исправлены замечания adversarial review Grok. Убран `TARGET_PROFILE`/`get_active_profile_name()` guard, первым фильтром стал только `chat_id=120363179621030401@g.us`; плагин включён в primary/default `~/.hermes/config.yaml`, где живёт WhatsApp. `docker inspect` оставлен с `timeout=2`, добавлен module-level IP cache TTL 300 сек и fallback последнего успешного IP в `~/.hermes/plugins/alikhan-sandbox-raw-ingest/db_host.json`, без hardcoded `172.22.0.4`; PostgreSQL подключается с `connect_timeout=2`, вся DB-работа fail-silent. `flock` теперь держится только на чтение/запись `seen.json`, не на INSERT. `SEEN_LIMIT=10000`; перед INSERT добавлен DB-дедуп `tags->>'mid'` + `chat_id`, `mid` пишется в `tags`. Для media без текста content берётся из `caption → filename → media_urls[0] → <media:type>`, `file_name` сохраняется в колонку и tags. Удалены `ALIKHAN_BOT_DIR`/`sys.path.insert`; пароль читается из `EVO_DB_PASS`, затем `~/.hermes/.env:EVO_DB_PASS`, затем совместимый fallback существующего `~/.hermes/secrets.env:DB_PASS` без вывода секрета. Проверка: `python3 -m py_compile ~/.hermes/plugins/alikhan-sandbox-raw-ingest/__init__.py` PASS; fake Event sandbox text INSERT id=4998, повтор dedup skip; production и чужой chat дали 0 строк; image с пустым text и caption `zz-caption-фото кота` записал content=caption и `file_name=cat-photo.jpg`; `EVO_DB_PASS` unset + file secret дал INSERT id=5000; PG `DB_HOST=127.0.0.2` вернул `None` за 0.001 сек; отдельный тест после очистки `seen.json` подтвердил DB-дедуп по `mid`; temp `.env` с `EVO_DB_PASS` при пустом env дал INSERT id=5004; тестовые строки удалены, `seen.json=[]`; финальный grep default `plugins.enabled` показывает `alikhan-sandbox-raw-ingest`. Файлы: `~/.hermes/plugins/alikhan-sandbox-raw-ingest/__init__.py`, `~/.hermes/plugins/alikhan-sandbox-raw-ingest/plugin.yaml`, `~/.hermes/config.yaml`, `CHRONOLOGY.md`.
- **04.09.2026 10:57** — chrono: gateway-адаптер sandbox-raw-ingest (канон Б) — плагин + 2 раунда review (`55c7bd0`)
- **04.09.2026 ~11:15** — Diamond-закрытие дня: (1) buzz-гвард Alikhan починен через 3 раунда Codex(Maker)+Grok(Checker): r1 substring-патч отклонён (traverse/подстроки/инъекции), r2 argv-allowlist отклонён Grok (4 CRITICAL: фейк-интерпретатор, sender-fallback через рассинхрон парсера, --channel подмена, $VAR через bash), r3 approved: только абсолютный /usr/bin/python3 + канонический buzz-send.py, bare python3 DENY (PATH-hijack в command-string guard не лечится), self-test 31/31; канон в AGENTS.md профиля обновлён (2 места). (2) gateway-адаптер канон Б (MASTER_SPEC:74,93 — сырьё из обеих групп): юзер-плагин ~/.hermes/plugins/alikhan-sandbox-raw-ingest/ (hook pre_gateway_dispatch), r1 Codex → r2 Grok NOT-APPROVED (1 блокер: sync docker inspect + PG без timeout мог заморозить event loop gateway; HIGH: гвард по профилю убивал плагин — WhatsApp живёт на default) → r4 Codex закрыл: connect_timeout=2, IP-cache TTL300 + db_host.json fallback, flock только на seen.json, гвард только по chat_id песочницы 120363179621030401@g.us, DB-дедуп tags->>mid + SEEN_LIMIT 10000, media-fallback caption→filename→URL, включён в default plugins.enabled. Independent verify: sandbox-INSERT VERIFIED, дедуп VERIFIED, боевая 0 строк, тесты почищены, после рестарта gateway 11:00 registered pre_gateway_dispatch (journal _PID=2151026). Коммиты: 8034650 (authority.py+watchdog_bridge v2), 55c7bd0 (chrono). E2E канона Б: ЖДЁТ живого сообщения в песочницу (пользователь не отправил). Открытый HIGH вне скоупа: gateway-adapter ACK-ает обе collect_only_chats включая боевую (класс 21.08) — потенциальный steal у диспетчера, требует отдельного решения. Урок: self-test агента ≠ верификация (r1: 12/12 при 4 критических обходах); substring-гварды не ремонтируются — только argv+realpath; bare-интерпретатор в allowlist = hijack.
- **04.09.2026 11:45** — chrono: 04.09 Diamond — buzz-гвард r1-r3 + gateway-адаптер канон Б (r1-r4), коммиты 8034650/55c7bd0, E2E ждёт живого сообщения (`261fb85`)
- **04.09.2026 11:47** — chrono: финальная правка 04.09 — append Diamond-запись (`91fceb7`)
...
- **05.09.2026 03:40** — chrono: 2726b58 (финальная запись, без самопорождённой следующей) (`c28df01`)
...
- **06.09.2026 14:07** — [drift-intent] SDG v1.4 DAG E2E (`eb45fef`)
- **06.09.2026 14:08** — [drift-intent] SDG v1.4 DAG E2E corrected (`fb98e9d`)
...
- **06.09.2026 14:12** — [drift-intent] SDG v1.4 cleanup (`ccb051d`)
- **06.09.2026 14:12** — SDG v1.4 E2E: cleanup probe files (`2891dea`)
...
- **08.09.2026 18:39** — SDG: writeback 5f63469 (`0f5744b`)
- **08.09.2026 20:23** — GWT: phase 4.4 — data-ingestion no-ci scenario + paths (`83e4813`)
- **08.09.2026 20:23** — SDG: writeback 83e4813 (`4acacd4`)
- **08.09.2026 20:24** — GWT: phase 4.4 — data-ingestion no-ci scenario + paths (`2cb9caa`)
- **08.09.2026 20:24** — SDG: writeback 2cb9caa (`0487be3`)
- **08.09.2026 20:25** — SDG: writeback 2cb9caa (`26348e0`)
...
- **09.09.2026 01:14** — SDG: writeback 1b88fe0 (`6b5372b`)
...
- **09.09.2026 03:00** — SDG: writeback 12f0383 (`ca61a3e`)
- **09.09.2026 04:02** — GWT: debt closure — authority+extractor 8 CI scenarios, no-ci converted (`fd02792`)
- **09.09.2026 04:02** — SDG: writeback fd02792 (`4077692`)
- **09.09.2026 04:02** — SDG: writeback fd02792 (`aea1303`)
- **09.09.2026 04:04** — GWT: debt closure — authority+extractor 8 CI scenarios, no-ci converted (`8159145`)
- **09.09.2026 04:04** — SDG: writeback 8159145 (`8c2d917`)
- **09.09.2026 04:04** — SDG: writeback 8159145 (`65a8bcf`)
