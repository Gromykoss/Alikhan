...
def build_poll_summary(status):
    ...
    if residuals_updated:
        ...
        for r in residuals_updated:
            o = f"{r['residual_volume']:.0f}" if r['residual_volume'] == int(r['residual_volume']) else f"{r['residual_volume']:.1f}"
            a = f"{r['actual_today']:.0f}" if r['actual_today'] == int(r['actual_today']) else f"{r['actual_today']:.1f}"
