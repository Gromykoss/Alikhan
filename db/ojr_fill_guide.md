...
> ⚠️ **ИСТОРИЧЕСКИЙ ДОКУМЕНТ**: стадия проектирования (2026-07-18, v1.0.0). Модуль `ojr_sync.py`
> не был реализован; текущий синк facts→OJR распределён: `bot/db.py` (facts, weather) +
> `bot/whatsapp_commands.py` (фото/доки/pass_register). Актуальная карта проекта —
> `PROJECT_MEMORY_GRAPH.md`. Документ не является источником правды о текущем состоянии.
